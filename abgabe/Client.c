...
ack receive_ack(SOCKET *sock) {
	...
	if (rcReceive == SOCKET_ERROR) {
		...
		printf("received ack with SeqNo: %ld and checkSum %ld\n", recAck.seqNo, recAck.checkSum);
	}
	return recAck;
}
	...
	while (get_next_frame(&buffer, filePointer) != -1) {
		...
		while ((!successful) && (numberOfTimeouts <= 3)) {
			...
			if (numberOfTimeouts == 2) {
				...
				ack recAck = receive_ack(sock);
				long receivedChecksum = recAck.checkSum;
				recAck.checkSum = 0;
				long expectedChecksum = calcChecksum(*(unsigned short*)&recAck, sizeof(ack));
				printf("expected checksum: %ld", expectedChecksum);
				
				if (seqNo == recAck.seqNo && expectedChecksum == receivedChecksum) {
					successful = 1;
				}
				else {
					init_set_SET(&sock, &fdSet);
